...
public class ConfVM {
	...
	private void reset() throws IOException, DBException {
		...
		if (this.configFile.exists()) {
			...
			reloadedSyncConfiguration.setEtlRootDirectory(rootDirectory + FileUtilities.getPathSeparator() + "sync"
					+ FileUtilities.getPathSeparator() + "data");
			...
			reloadedSyncConfiguration.getSrcConnInfo()
					.setDataBaseUserName(properties.getProperty("connection.username"));
			reloadedSyncConfiguration.getSrcConnInfo()
					.setDataBaseUserPassword(properties.getProperty("connection.password"));
		}

		reloadedSyncConfiguration.setRelatedController(this.etlConfiguration.getRelatedController() == null
				? new ProcessController(null, reloadedSyncConfiguration)
				: this.etlConfiguration.getRelatedController());

		reloadedSyncConfiguration.setClassPath(java.util.Collections.singletonList(retrieveClassPath()));
