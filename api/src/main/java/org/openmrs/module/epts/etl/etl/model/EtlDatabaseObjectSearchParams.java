...
public class EtlDatabaseObjectSearchParams extends AbstractEtlSearchParams<EtlDatabaseObject> {
	...
	private void loadAuxExtractTable(AuxQueryInfo queryInfo, JoinableEntity aux, EtlDatabaseObject parentObject,
		...
		if (utilities.stringHasValue(extraJoinQuery)) {
			PreparedQuery pQ = PreparedQuery.prepare(QueryDataSourceConfig.fastCreate(extraJoinQuery, getSrcConf()),
					...
					EtlDataSource
							.extractDataSourceFromObjects(collectDataSourceObjects(parentObject, auxDataSourceObjects)),
					true, DbmsType.determineFromConnection(srcConn));

			PreparedQueryInfo pQInfo = pQ.generatePreparedQuery(this.getRelatedEtlConf(), null, parentObject,
					parentObject, auxDataSourceObjects, srcConn);

			Object[] params = pQInfo.extractParametersValueToArray();

			extraJoinQuery = pQInfo.getPreparedQuery();
	...
	public String generateDestinationExclusionClause(Connection srcConn, Connection dstConn) throws DBException {
		...
		if (!DBUtilities.isSameDatabaseServer(srcConn, dstConn)) {
			throw new ForbiddenOperationException(
					"The database server must be the same to generate exlusion clause!!!");
		}
	...
	private String generateDestinationJoinSubquery(DstConf dstConf, Connection dstConn) throws DBException {
		...
		if (utilities.stringHasValue(joinCondition)) {
			...
			throw new ForbiddenOperationException("There is no join condition between the src ["
					+ srcTabConf.getTableName() + "] and it destination table [" + dstConf.getTableName() + "]");
		}
