...
public class ConceptVO extends AbstractDatabaseObject implements EtlDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE concept SET `concept_id` = ?, `retired` = ?, `short_name` = ?, `description` = ?, `form_text` = ?, `datatype_id` = ?, `class_id` = ?, `is_set` = ?, `creator` = ?, `date_created` = ?, `version` = ?, `changed_by` = ?, `date_changed` = ?, `retired_by` = ?, `date_retired` = ?, `retire_reason` = ?, `uuid` = ? WHERE concept_1.concept_id = ? ";
	}
