...
public class PatientVO extends AbstractGeneratedDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE patient SET `patient_id` = ?, `creator` = ?, `date_created` = ?, `changed_by` = ?, `date_changed` = ?, `voided` = ?, `voided_by` = ?, `date_voided` = ?, `void_reason` = ?, `allergy_status` = ? WHERE patient_67.patient_id = ? ";
	}
