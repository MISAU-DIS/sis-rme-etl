...
public class PatientVO extends AbstractDatabaseObject implements EtlDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE patient SET `patient_id` = ?, `creator` = ?, `date_created` = ?, `changed_by` = ?, `date_changed` = ?, `voided` = ?, `voided_by` = ?, `date_voided` = ?, `void_reason` = ?, `allergy_status` = ? WHERE patient_1.patient_id = ? ";
	}
	...
	public void copyFrom(EtlDatabaseObject toCopyFrom) {
		if (toCopyFrom instanceof PatientVO) {
			PatientVO toCopyFromAsPatientVO = (PatientVO) toCopyFrom;

			this.patientId = copyGeneratedField(toCopyFromAsPatientVO.patientId);
			this.creator = copyGeneratedField(toCopyFromAsPatientVO.creator);
			this.dateCreated = toCopyFromAsPatientVO.dateCreated;
			this.changedBy = copyGeneratedField(toCopyFromAsPatientVO.changedBy);
			this.dateChanged = toCopyFromAsPatientVO.dateChanged;
			this.voided = copyGeneratedField(toCopyFromAsPatientVO.voided);
			this.voidedBy = copyGeneratedField(toCopyFromAsPatientVO.voidedBy);
			this.dateVoided = toCopyFromAsPatientVO.dateVoided;
			this.voidReason = copyGeneratedField(toCopyFromAsPatientVO.voidReason);

			if (getSharedPkObj() != null && toCopyFromAsPatientVO.getSharedPkObj() != null) {
				getSharedPkObj().copyFrom(toCopyFromAsPatientVO.getSharedPkObj());
			}
		}
	}
