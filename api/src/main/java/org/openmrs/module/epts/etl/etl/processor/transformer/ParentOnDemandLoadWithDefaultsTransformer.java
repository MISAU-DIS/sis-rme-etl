...
/**
 * Specialized version of {@link ParentOnDemandLoadTransformer} that enables the
 * use of default field values when creating parent records on demand.
 * <p>
 * This transformer behaves like {@link ParentOnDemandLoadTransformer}, but
 * automatically configures the on-demand parent creation process to use default
 * values for destination fields that were not explicitly mapped.
 * </p>
 * <p>
 * Internally, this transformer sets the unmapped field behavior to
 * {@code USE_DEFAULT}, allowing values from the global
 * {@code defaultFieldValues} configuration to be applied when required.
 * </p>
 * <p>
 * This is useful when creating placeholder or default parent records in the
 * destination database, especially when some required destination fields are
 * not provided in the transformer configuration.
 * </p>
 *
...
public class ParentOnDemandLoadWithDefaultsTransformer extends ParentOnDemandLoadTransformer {
	...
	public static ParentOnDemandLoadWithDefaultsTransformer getInstance(List<Object> parameters,
		...
		try {
			ParentOnDemandLoadWithDefaultsTransformer created = new ParentOnDemandLoadWithDefaultsTransformer(
					OnDemandInfo.create(effectiveParameters, (DstConf) relatedEtlTransformTarget, field, conn), conn);

			ParentOnDemandLoadWithDefaultsTransformer previous = INSTANCES.putIfAbsent(key, created);

			return previous != null ? previous : created;
		} catch (DBException | FieldAvaliableInMultipleDataSources e) {
