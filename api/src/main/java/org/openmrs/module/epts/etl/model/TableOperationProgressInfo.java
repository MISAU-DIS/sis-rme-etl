...
public class TableOperationProgressInfo extends BaseVO {
	...
	/*
	 * Since in destination site the etlItemConfiguration is aplayed to all sites,
	 * then it is needed to fix it to allow manual specification
	 */
	...
	public void load(ResultSet resultSet) throws SQLException {
		...
		this.progressMeter = EtlProgressMeter.fullInit(status, startTime, stopTime, processing_time, pause_time,
				minRecordId, maxRecordId, total, processed);
	}
	...
	public synchronized void save(Connection conn) throws DBException {
		TableOperationProgressInfo recordOnDB = TableOperationProgressInfoDAO.find(this.controller,
				getEtlConfiguration(), conn);
