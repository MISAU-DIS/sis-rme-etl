...
public class EtlProgressMeter implements TimeCountDownInitializer {
	...
	/**
	 * The last analyzed record in interval {@link #minRecordId} -
	 * {@link #maxRecordId}
	 */
	 ...
	 * <p>
	 * Com base no valor deste atributo pode se evitar o "refresh" sempre que o
	 * mesmo se encontrar updated
	 */
	private boolean updated;

	/**
	 * Indica o intervalo de tempo durante o qual este {@link EtlProgressMeter} sera
	 * considerado updated
	 */
	...
	public EtlProgressMeter(String statusMsg, long minRecordId, long maxRecordId, int total, int processed,
		...
		// Assume-se que por omissao qualquer meter encontra-se desactualizado, de tal
		// maneiras que
	...
	public double getProgressOfAnalyzedRecords() {
		...
		if (this.getTotalAnalyzed() > 0) {
			...
			progress = Double
					.parseDouble(utilities.getNumberInXPrecision((processedAsDouble / totalAsDouble) * 100, 2));
		}
	 ...
	 * <p>
	 * Cada meter pode definir um intervalo dentro do qual o mesmo pode ser
