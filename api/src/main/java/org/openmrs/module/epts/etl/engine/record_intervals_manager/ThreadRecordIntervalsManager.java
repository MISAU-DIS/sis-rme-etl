 ...
 * {@link IntervalExtremeRecord}. For each processing iteration, there is
 * current processing interval defined by {@link ThreadCurrentIntervals} and
 * this allow easy moving to the next interval
 * 
 * @author jpboane
 */
public class ThreadRecordIntervalsManager<T extends EtlDatabaseObject> extends IntervalExtremeRecord
		implements Comparable<ThreadRecordIntervalsManager<T>> {
		...
		if (currentFirstRecordId > this.getMaxRecordId()) {
			...
			if (currentLastRecordId < currentFirstRecordId) {
				...
				this.setCurrentLimits(new ThreadCurrentIntervals(currentFirstRecordId, currentLastRecordId,
						this.getEngine().getParallelProcessingStrategy().usesSharedTransformationQueue()
								|| this.getEngine().getParallelProcessingStrategy().isSingleThread() ? 1
	...
	public synchronized void save() {
		...
		if (isOutOfLimits() || isNotInitialized()) {
			...
			setLastSavedOn(
					TaskProcessor.utilities.formatDateToDDMMYYYY_HHMISS(TaskProcessor.utilities.getCurrentDate()));
	...
	/**
	 * Verifica se os limites ainda permitem pesquisa. Os limites deixam de permitir
	 * pesquisa quando o valor de {@link #getCurrentFirstRecordId()} for inferior ou
	 * igual ao valor de {@link #getCurrentLastRecordId()}
	 * 
	 * @return
	 */
	...
	public synchronized void moveNext() {
		if (canGoNext()) {
			...
			throw new ForbiddenOperationException("You reached the max dstRecord. Curr Status: ["
					+ this.getMinRecordId() + " - " + this.getMaxRecordId() + "] Curr ["
	...
	/**
	 * Tries to load data for this processor from file. If there is no saved limits
	 * then will keeped the current limits info
	 * 
	 * @param file
	 * @param processor
	 */
		...
		try {
			...
			if (limits != null) {
				...
				if (limits.hasFinalCheckIntervalsManager()) {
					...
					limits.getFinalCheckIntervalsManager()
							.setMaxSupportedProcessors(engine.getMaxSupportedProcessors());
					limits.getFinalCheckIntervalsManager()
							.setQtyRecordsPerProcessing(engine.getMaxRecordsPerProcessing());
				}
			}
