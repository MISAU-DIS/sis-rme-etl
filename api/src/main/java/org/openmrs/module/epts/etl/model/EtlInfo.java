...
public class EtlInfo extends AbstractEtlDataConfiguration {
	...
	public void reloadParentsWithDefaultValues(Connection srcConn, Connection dstConn)
		...
		for (ParentInfo parentInfo : this.getParentsWithDefaultValues()) {
			...
			List<EtlDatabaseObject> recursive = retriveRecursiveRelationship(parentInfo.getParentTableConfInDst(),
					srcConn, dstConn);
						...
						try {
							...
							if (recordAsSrc.getEtlInfo().getResultItem().hasInconsistences()
								...
								getProcessor().logDebug(
										"The parent for default for parent [" + parentInfo.getParentRecordInOrigin()
												+ "] could not be loaded. The dstRecord [");

								this.getResultItem()
										.addInconsistence(InconsistenceInfo.generate(getTransformedObject(),
												parentInfo.getParentTableConfInDst(),
		...
		for (ParentTable refInfo : getDstConf().getParentRefInfo()) {
			...
			if (this.relationshipResolutionStrategy().validateOnly()) {
				...
				if (!skipDstParentLoad) {
					...
					if (parentInSrc != null) {
						parentInDst = this.getTransformedObject().retrieveParentInDestination(refInfo, parentInSrc,
								dstConn);
					} else {
						...
						try {
							...
							this.getResultItem().addInconsistence(InconsistenceInfo.generate(getTransformedObject(),
									refInfo, this.getDstConf().getOriginAppLocationCode()));
						}
					}
	 ...
	 * 
	 * @param refInfo
	 * @return true if this record fulfill the conditions for parent or if the
	 *         parent is not conditional
	 */
	...
	public List<EtlDatabaseObject> retriveRecursiveRelationship(ParentTable refInfo, Connection srcConn,
		...
		if (parent == null) {
			...
			/*
			 * Check if there are recursive relationship between the child (this record) and
			 * its parent
			 * 
			...
			for (ParentTable p : refInfo.getParentRefInfo()) {
				...
				if (p.getTableName().equals(this.getDstConf().getTableName())) {
					 ...
					 * 
					 * if (parentValue != null &&
					 * parentValue.equals(this.getSrcRecord().getObjectId().asSimpleValue())) {
					 * recursiveRelationship.add(RecordWithDefaultParentInfo.init(this.getSrcRecord(
					 * ), this.getDstRecord(), parentInOrigin, refInfo, srcConn));
					 * 
					 * break; }
					 */
				}
	...
	public List<EtlDatabaseObject> retriveRecursiveRelationship(Connection srcConn, Connection dstConn)
			throws DBException {
		if (!utilities.listHasElement(getDstConf().getParentRefInfo())) {
			return null;
		}
	...
	public void saveRecordsWithDefaultsParents(Connection srcConn, Connection dstConn) throws DBException {
		for (ParentInfo parentInfo : this.getParentsWithDefaultValues()) {
			RecordWithDefaultParentInfo defaultParentInfo = RecordWithDefaultParentInfo.init(this.getRelatedSrcObject(),
					this.getTransformedObject(), parentInfo.getParentRecordInOrigin(),
					parentInfo.getParentTableConfInDst(), srcConn);
	...
	public static EtlInfo initEtlRecord(EtlProcessor processor, EtlDatabaseObject srcObject,
			EtlDatabaseObject destObject) {
