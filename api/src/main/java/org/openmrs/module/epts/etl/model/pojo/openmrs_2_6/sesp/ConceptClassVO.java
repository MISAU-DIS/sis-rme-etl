...
public class ConceptClassVO extends AbstractDatabaseObject implements EtlDatabaseObject {
	private Field conceptClassId = Field.fastCreateWithType("concept_class_id", "INT");
	...
	public void setConceptClassId(Field conceptClassId) {
		this.conceptClassId = conceptClassId;
	}

	public void setConceptClassIdValue(Integer value) {
		this.conceptClassId.setValue(value);
	}

	public Field getConceptClassId() {
	...
	public void load(ResultSet rs) throws SQLException {
		...
		String conceptClassIdAttName = utilities.concatStringsWithSeparator(this.getRelatedConfiguration().getAlias(),
				"concept_class_id", "_");

		this.conceptClassId.setValue(BaseVO.retrieveFieldValue(conceptClassIdAttName, "INT", rs));
	...
	public String getInsertSQLWithoutObjectId() {
		return "INSERT INTO concept_class(`name`, `description`, `creator`, `date_created`, `retired`, `retired_by`, `date_retired`, `retire_reason`, `uuid`, `date_changed`, `changed_by`) VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	}

	@JsonIgnore
	@Override
	public String getInsertSQLWithObjectId() {
		return "INSERT INTO concept_class(`concept_class_id`, `name`, `description`, `creator`, `date_created`, `retired`, `retired_by`, `date_retired`, `retire_reason`, `uuid`, `date_changed`, `changed_by`) VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	}
	...
	public Object[] getInsertParamsWithObjectId() {
		Object[] params = { this.conceptClassId.getValue(), this.name.getValue(), this.description.getValue(),
				this.creator.getValue(), this.dateCreated, this.retired.getValue(), this.retiredBy.getValue(),
				this.dateRetired.getValue(), this.retireReason.getValue(), this.uuid, this.dateChanged,
	...
	public Object[] getUpdateParams() {
		Object[] params = { this.conceptClassId.getValue(), this.name.getValue(), this.description.getValue(),
				this.creator.getValue(), this.dateCreated, this.retired.getValue(), this.retiredBy.getValue(),
				this.dateRetired.getValue(), this.retireReason.getValue(), this.uuid, this.dateChanged,
				this.changedBy.getValue(), this.conceptClassId.getValue() };
		return params;
	}
	...
	public String getUpdateSQL() {
		return "UPDATE concept_class SET `concept_class_id` = ?, `name` = ?, `description` = ?, `creator` = ?, `date_created` = ?, `retired` = ?, `retired_by` = ?, `date_retired` = ?, `retire_reason` = ?, `uuid` = ?, `date_changed` = ?, `changed_by` = ? WHERE concept_class_1.concept_class_id = ? ";
	}
	...
	public String generateInsertValuesWithObjectId() {
		return "" + (this.conceptClassId.getValue()) + ","
				+ (this.name.getValue() != null
						? "\"" + utilities.scapeQuotationMarks(this.name.getValue().toString()) + "\""
						: null)
				+ ","
	...
	public EtlDatabaseObject createACopy() {
		ConceptClassVO copy = new ConceptClassVO();

		copy.conceptClassId = copyGeneratedField(this.conceptClassId);
	...
	public void copyFrom(EtlDatabaseObject toCopyFrom) {
		if (toCopyFrom instanceof ConceptClassVO) {
			ConceptClassVO toCopyFromAsConceptClassVO = (ConceptClassVO) toCopyFrom;

			this.conceptClassId = copyGeneratedField(toCopyFromAsConceptClassVO.conceptClassId);
			this.name = copyGeneratedField(toCopyFromAsConceptClassVO.name);
			this.description = copyGeneratedField(toCopyFromAsConceptClassVO.description);
			this.creator = copyGeneratedField(toCopyFromAsConceptClassVO.creator);
			this.dateCreated = toCopyFromAsConceptClassVO.dateCreated;
			this.retired = copyGeneratedField(toCopyFromAsConceptClassVO.retired);
			this.retiredBy = copyGeneratedField(toCopyFromAsConceptClassVO.retiredBy);
			this.dateRetired = copyGeneratedField(toCopyFromAsConceptClassVO.dateRetired);
			this.retireReason = copyGeneratedField(toCopyFromAsConceptClassVO.retireReason);
			this.uuid = toCopyFromAsConceptClassVO.uuid;
			this.dateChanged = toCopyFromAsConceptClassVO.dateChanged;
