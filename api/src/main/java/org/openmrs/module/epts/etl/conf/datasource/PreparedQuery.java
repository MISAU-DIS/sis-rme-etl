 ...
 * and its parameters
 */
public class PreparedQuery extends AbstractEtlDataConfiguration {
			...
			if (utilities.listHasExactlyOneElement(this.getDataSource().getFields())
				...
				if (avaliableFunction.get(0).isCountFunction()) {
					...
					if (mainTableName.startsWith("@")) {
						// mainTableName =
						// retrieveParamValue(utilities.removeFirsChar(mainTableName)).toString();
					}
	...
	/**
	 * Extract all the parameters presents in a dump query. This assume that the
	...
	public EtlConfiguration getRelatedEtlConf() {
		return this.getDataSource().getRelatedEtlConf();
	}
