...
public interface TableConfiguration extends EtlDatabaseObjectConfiguration, EtlDataSource {
	...
	public static final String STAGE_TABLE_PROCESSED_SUFIX = "_processed_rec";
	...
	/**
	 * If present, the value from this method will be mapped as a primary key for
	 * this table if the table does not have a primary key but have a field with
			...
			if (!DBUtilities.isTableExists(this.getSyncStageSchema(), this.getTableName(), conn)) {
				...
				for (Field f : this.getFields()) {
					...
					if (f.isClob()) {
						...
						sql += DBUtilities.generateTableVarcharField(f.getName(), f.getPrecision().getLength(),
								constraint, conn) + endLineMarker;
						...
						sql += DBUtilities.generateTableIntegerField(f.getName(), f.getPrecision().getLength(),
								constraint, conn) + endLineMarker;
	...
	default void clone(TableConfiguration toCloneFrom, EtlDataConfiguration parent, EtlDatabaseObject schemaInfoSrc,
		...
		if (toCloneFrom.hasExtraConditionForExtract() && !toCloneFrom.isUsingManualDefinedAlias()) {
			...
			this.setExtraConditionForExtract(this.getExtraConditionForExtract()
					.replaceAll(toCloneFrom.getTableName() + "\\.", getTableAlias() + "\\."));
	...
	default String generateSkippedRecordInclusionClause() {
		...
		EtlConfigurationTableConf recWithDefaultParents = this.getRelatedEtlConf()
	...
	default Boolean checkIfisIgnorableParentByClassAttName(String parentAttName, Connection conn) {
		...
		throw new ForbiddenOperationException(
				"The att '" + parentAttName + "' doesn't represent any defined parent att");
	}
										...
										for (ParentTable manualConfiguredRefInfo : this.getParents()) {
											...
											if (autoLoadedRefInfo.getTableName()
												...
												if (!manualConfiguredRefInfo.hasMapping()) {
													...
													mixedConfiguredRef
															.setRefMapping(autoLoadedRefInfo.cloneAllMapping());

													mixedConfiguredRef.getSimpleRefMapping()
															.setDefaultValueDueInconsistency(manualConfiguredRefInfo
												...
												if (autoLoadedRefInfo.equals(mixedConfiguredRef)) {
													autoLoadedRefInfo.setConditionalFields(
															mixedConfiguredRef.getConditionalFields());
		...
		synchronized (this) {
			...
			if (count == 0) {
				...
				try {
					...
					while (foreignKeyRS.next()) {
						logDebug("CONFIGURING CHILD " + ++i + " [" + foreignKeyRS.getString("FKTABLE_NAME")
								+ "] FOR TABLE '" + this.getTableName() + "'");
	...
	@JsonIgnore
	default String generateRelatedProcessedRecordStageTableName() {
		return this.getTableName() + STAGE_TABLE_PROCESSED_SUFIX;
	}
	...
	@JsonIgnore
	default String generateFullProcessedRecordStageTableName() {
		return this.getSyncStageSchema() + "." + this.generateRelatedProcessedRecordStageTableName();
	}
	...
	default Boolean existRelatedProcessedRecordStageTable(Connection conn) {
		return existStageTable(this.generateRelatedProcessedRecordStageTableName(), conn);
	}
		...
		synchronized (this) {
			...
			try {
				...
				if (this.hasExtraConditionForExtract()) {
					this.setExtraConditionForExtract(this.getExtraConditionForExtract()
							.replaceAll(getTableName() + "\\.", this.getTableAlias() + "\\."));
				}
	...
	default Boolean overrideAutoIncrement() {
		return this.getAutoIncrementHandlingType() != null
				&& this.getAutoIncrementHandlingType().isIgnoreSchemaDefinition();
	}
	...
	default String generateUpdateSQL() {
		...
		if (this.getPrimaryKey() != null) {
			...
			updateSQL += " WHERE "
					+ UniqueKeyInfo.parseToParametrizedStringConditionToAllWithoutAlias(this.getUniqueKeys());
		}
	...
	default String generateFullAliasedSelectColumns() throws ForbiddenOperationException {
		...
		for (Field f : this.getFields()) {
			fullSelectColumns = utilities.concatStringsWithSeparator(fullSelectColumns,
					f.generateAliasedSelectColumn(this), ",\n");
		}
	...
	/**
	 * Usually called after a call to {@link #fullLoad()}, allow the loading of own
	 * elements. Eg. the join fields,etc
	 */
	...
	default ParentTable findParentRefInfoByParentTable(String parentTable) {
		...
		if (utilities.listHasElement(parents)) {
			...
			throw new EtlExceptionImpl("Multiple references to parent '" + parentTable + "' found within table '"
					+ this.getTableName() + "'");
		}
	...
	default EtlConfigurationTableConf generateRelatedSrcStageTableConf(Connection conn) throws DBException {
		...
		return this.generateRelatedStageTabConf(this.generateRelatedSrcStageTableName(), this.getSyncStageSchema(),
				conn);
	}

	default EtlConfigurationTableConf generateRelatedProcessedRecordStageTableConf(Connection conn) throws DBException {
		if (!existRelatedProcessedRecordStageTable(conn)) {
			throw new ForbiddenOperationException("The table " + this.generateRelatedProcessedRecordStageTableName()
					+ " does not exists! Ensure that the DB_PREPARATION operation was full executed!");
		}

		return this.generateRelatedStageTabConf(this.generateRelatedProcessedRecordStageTableName(),
				this.getSyncStageSchema(), conn);
	}
	...
	default EtlConfigurationTableConf generateRelatedDstStageTableConf(Connection conn) throws DBException {
		...
		return this.generateRelatedStageTabConf(this.generateRelatedDstStageTableName(), this.getSyncStageSchema(),
				conn);
	}
	...
	default EtlConfigurationTableConf generateRelatedStageDstUniqueKeysTableConf(Connection conn) throws DBException {
		...
		return this.generateRelatedStageTabConf(this.generateRelatedStageDstUniqueKeysTableName(),
				this.getSyncStageSchema(), conn);
	}
	...
	default EtlConfigurationTableConf generateRelatedStageSrcUniqueKeysTableConf(Connection conn) throws DBException {
		...
		return this.generateRelatedStageTabConf(this.generateRelatedStageSrcUniqueKeysTableName(),
				this.getSyncStageSchema(), conn);
	}
	...
	default void createRelatedDstSyncStage(Connection conn) throws DBException {
		...
		synchronized (lock) {
			if (!DBUtilities.isTableExists(this.getSyncStageSchema(), this.generateRelatedDstStageTableName(), conn)) {
				...
				sql += DBUtilities.generateTableVarcharField("dst_table_name", 250, notNullConstraint, conn)
						+ endLineMarker;
				...
				sql += DBUtilities.generateTableVarcharField("src_stage_table_name", 100, notNullConstraint, conn)
						+ endLineMarker;
				sql += DBUtilities.generateTableVarcharField("etl_confing_id", 250, nullConstraint, conn)
						+ endLineMarker;
				...
				sql += DBUtilities.generateTableVarcharField("last_sync_try_err", 500, nullConstraint, conn)
						+ endLineMarker;
				sql += DBUtilities.generateTableNumericField("consistent", 1, nullConstraint, -1, conn) + endLineMarker;
				sql += DBUtilities.generateTableNumericField("migration_status", 1, nullConstraint, 1, conn)
						+ endLineMarker;

				sql += DBUtilities.generateTableDateTimeFieldWithDefaultValue("creation_date", conn) + endLineMarker;

				String checkCondition = "migration_status IN (-1,0,1,2,3,4)";
				String keyName = "CHK_" + tableName + "_MIG_STATUS".toUpperCase();

				sql += DBUtilities.generateTableCheckConstraintDefinition(keyName, checkCondition, conn)
						+ endLineMarker;
				...
				String idxDefinition1 = DBUtilities.generateIndexDefinition(fullTableName, indexName1, indexFields1,
						conn);

				String indexName2 = tableName + "_src_rec_idx";
				String indexFields2 = "stage_record_id, dst_table_name";

				String idxDefinition2 = DBUtilities.generateIndexDefinition(fullTableName, indexName2, indexFields2,
						conn);
	...
	default void createRelatedSyncStageAreaUniqueKeysTable(String tableName, String parentTableName, Connection conn)
		...
		synchronized (lock) {
			if (!DBUtilities.isTableExists(this.getSyncStageSchema(), tableName, conn)) {
				...
				sql += DBUtilities.generateTableVarcharField("column_name", 100, notNullConstraint, conn)
						+ endLineMarker;
	...
	default void createRelatedProcessedRecordStageAreaTable(Connection conn) throws DBException {

		synchronized (lock) {
			if (DBUtilities.isMySQLDB(conn)) {
				String stageTableName = this.generateRelatedProcessedRecordStageTableName();

				if (DBUtilities.isTableExists(this.getSyncStageSchema(), stageTableName, conn)) {
					return;
				}

				String fullStageTableName = this.generateFullProcessedRecordStageTableName();
				...
				throw new EtlExceptionImpl("Unsupported database engine for this operation!!!");
			}
		}
	}
	...
	default void createRelatedSrcStageAreaTable(Connection conn) throws DBException {
		...
		synchronized (lock) {
			if (!DBUtilities.isTableExists(this.getSyncStageSchema(), this.generateRelatedSrcStageTableName(), conn)) {
				...
				sql += DBUtilities.generateTableAutoIncrementField("id", conn) + endLineMarker;
				sql += DBUtilities.generateTableVarcharField("record_origin_location_code", 100, notNullConstraint,
						conn) + endLineMarker;
				...
				sql += DBUtilities.generateTableDateTimeField("last_sync_date", nullConstraint, conn) + endLineMarker;
				sql += DBUtilities.generateTableVarcharField("last_sync_try_err", 250, nullConstraint, conn)
						+ endLineMarker;
				...
				sql += DBUtilities.generateTableDateTimeField("record_date_created", nullConstraint, conn)
						+ endLineMarker;
				sql += DBUtilities.generateTableDateTimeField("record_date_changed", nullConstraint, conn)
						+ endLineMarker;
				sql += DBUtilities.generateTableDateTimeField("record_date_voided", nullConstraint, conn)
						+ endLineMarker;
				...
				String idxDefinition = DBUtilities.generateIndexDefinition(this.generateFullSrcStageTableName(),
						indexName, indexFields, conn);
