...
public class VisitVO extends AbstractGeneratedDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE visit SET `visit_id` = ?, `patient_id` = ?, `visit_type_id` = ?, `date_started` = ?, `date_stopped` = ?, `indication_concept_id` = ?, `location_id` = ?, `creator` = ?, `date_created` = ?, `changed_by` = ?, `date_changed` = ?, `voided` = ?, `voided_by` = ?, `date_voided` = ?, `void_reason` = ?, `uuid` = ? WHERE visit_45.visit_id = ? ";
	}
