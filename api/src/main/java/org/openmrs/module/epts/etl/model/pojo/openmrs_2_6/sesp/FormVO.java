...
public class FormVO extends AbstractDatabaseObject implements EtlDatabaseObject { 
	...
	public String getUpdateSQL(){ 
 		return "UPDATE form SET `form_id` = ?, `name` = ?, `version` = ?, `build` = ?, `published` = ?, `description` = ?, `encounter_type` = ?, `template` = ?, `xslt` = ?, `creator` = ?, `date_created` = ?, `changed_by` = ?, `date_changed` = ?, `retired` = ?, `retired_by` = ?, `date_retired` = ?, `retired_reason` = ?, `uuid` = ? WHERE form_3.form_id = ? "; 
	} 
