...
public class OrderTypeVO extends AbstractGeneratedDatabaseObject {
	...
	public OrderTypeVO() {
		...
		this.fields.add(this.javaClassName);
		this.fields.add(this.parent);
		this.fields.add(this.changedBy);
	}

	@Override
	public void tryToReplaceFieldValueWithKeyValue(Key k) {
		if (utilities.equalsFieldsName(k.getName(), "order_type_id")) {
			this.orderTypeId.setValue(k.getValue());
		}
	}
	...
	public void setFieldValue(String fieldName, Object value) {
		...
		if (utilities.equalsFieldsName(fieldName, "java_class_name")) {
			this.javaClassName.setValue(value instanceof Field ? ((Field) value).getValue() : value);
			regenerateObjectIdIfKeyField(fieldName);
			return;
		}
		if (utilities.equalsFieldsName(fieldName, "parent")) {
			this.parent.setValue(value instanceof Field ? ((Field) value).getValue() : value);
			regenerateObjectIdIfKeyField(fieldName);
			return;
		}
		if (utilities.equalsFieldsName(fieldName, "changed_by")) {
	...
	public void loadWithDefaultValues(Connection srcConn, Connection dstConn) throws DBException {
		...
		loadGeneratedFieldWithDefaultValue(this.javaClassName, srcConn, dstConn);
		loadGeneratedFieldWithDefaultValue(this.parent, srcConn, dstConn);
