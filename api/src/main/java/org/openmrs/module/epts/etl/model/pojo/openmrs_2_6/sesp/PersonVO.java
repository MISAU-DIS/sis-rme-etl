...
public class PersonVO extends AbstractDatabaseObject implements EtlDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE person SET `person_id` = ?, `gender` = ?, `birthdate` = ?, `birthdate_estimated` = ?, `dead` = ?, `death_date` = ?, `cause_of_death` = ?, `creator` = ?, `date_created` = ?, `changed_by` = ?, `date_changed` = ?, `voided` = ?, `voided_by` = ?, `date_voided` = ?, `void_reason` = ?, `uuid` = ?, `deathdate_estimated` = ?, `birthtime` = ?, `cause_of_death_non_coded` = ? WHERE person_2.person_id = ? ";
	}
	...
	public void copyFrom(EtlDatabaseObject toCopyFrom) {
		if (toCopyFrom instanceof PersonVO) {
			PersonVO toCopyFromAsPersonVO = (PersonVO) toCopyFrom;

			this.personId = copyGeneratedField(toCopyFromAsPersonVO.personId);
			this.gender = copyGeneratedField(toCopyFromAsPersonVO.gender);
			this.birthdate = copyGeneratedField(toCopyFromAsPersonVO.birthdate);
			this.birthdateEstimated = copyGeneratedField(toCopyFromAsPersonVO.birthdateEstimated);
			this.dead = copyGeneratedField(toCopyFromAsPersonVO.dead);
			this.deathDate = copyGeneratedField(toCopyFromAsPersonVO.deathDate);
			this.causeOfDeath = copyGeneratedField(toCopyFromAsPersonVO.causeOfDeath);
			this.creator = copyGeneratedField(toCopyFromAsPersonVO.creator);
			this.dateCreated = toCopyFromAsPersonVO.dateCreated;
			this.changedBy = copyGeneratedField(toCopyFromAsPersonVO.changedBy);
			this.dateChanged = toCopyFromAsPersonVO.dateChanged;
			this.voided = copyGeneratedField(toCopyFromAsPersonVO.voided);
			this.voidedBy = copyGeneratedField(toCopyFromAsPersonVO.voidedBy);
			this.dateVoided = toCopyFromAsPersonVO.dateVoided;
			this.voidReason = copyGeneratedField(toCopyFromAsPersonVO.voidReason);
			this.uuid = toCopyFromAsPersonVO.uuid;
			this.deathdateEstimated = copyGeneratedField(toCopyFromAsPersonVO.deathdateEstimated);
			this.birthtime = copyGeneratedField(toCopyFromAsPersonVO.birthtime);
