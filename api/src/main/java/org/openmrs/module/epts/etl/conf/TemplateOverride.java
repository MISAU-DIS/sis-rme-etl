...
public class TemplateOverride extends AbstractEtlDataConfiguration {
	...
	private Object readFieldValue(Object target, String fieldName) {
		try {
			...
			if (field == null) {
				throw new EtlExceptionImpl(
						"Field '" + fieldName + "' not found on class " + target.getClass().getName());
			}
			...
			throw new EtlExceptionImpl(
					"Error reading field '" + fieldName + "' from class " + target.getClass().getName(), e);
		}
	}
	...
	private ResolvedPath resolveParentAndField(Object root, String path) {
		...
		if (root == null) {
			throw new EtlExceptionImpl("Root object cannot be null while resolving path '" + path + "'");
		}
		...
		for (int i = 0; i < parts.length - 1; i++) {
			...
			if (field == null) {
				throw new EtlExceptionImpl("Field '" + part + "' not found while resolving path '" + path
						+ "' on class " + currentObject.getClass().getName());
			}
			...
			if (currentObject == null) {
				throw new EtlExceptionImpl(
						"Intermediate object '" + part + "' is null while resolving path '" + path + "'");
			}
			...
			if (currentObject instanceof List<?>) {
				...
				if (list.size() == 1) {
					...
					if (currentObject == null) {
						throw new EtlExceptionImpl(
								"The only element of list '" + part + "' is null while resolving path '" + path + "'");
					}

				} else {
					throw new EtlExceptionImpl(
							"Field '" + part + "' is a List with " + list.size() + " elements while resolving path '"
									+ path + "'. " + "Only Lists with exactly one element are supported "
									+ "when the List is not the last element of the path.");
				}
			}
		}
	...
	private Class<?> resolveListItemType(Field field) {
		...
		throw new EtlExceptionImpl(
				"Unsupported generic item type for list field '" + field.getName() + "': " + itemType);
	}
