...
public class UniqueKeyInfo implements Comparable<UniqueKeyInfo> {
	...
	public static List<UniqueKeyInfo> loadUniqueKeysInfo(TableConfiguration tabConf, Connection conn)
			throws DBException {
	...
	public String toString() {
		...
		if (hasFields()) {
			List<Field> sortedFields = new ArrayList<>(this.getFields());

			sortedFields
					.sort(Comparator.comparing(Field::getName, Comparator.nullsLast(String.CASE_INSENSITIVE_ORDER)));
	...
	public static List<UniqueKeyInfo> cloneAllAndLoadValues(List<UniqueKeyInfo> uniqueKeysInfo, EtlDatabaseObject obj) {
		// Since this is usually for an object, null the keyName to avoid "equals" 2
		// object because they have same keyName
