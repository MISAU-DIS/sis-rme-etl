...
public class ConceptVO extends AbstractDatabaseObject implements EtlDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE concept SET `concept_id` = ?, `retired` = ?, `short_name` = ?, `description` = ?, `form_text` = ?, `datatype_id` = ?, `class_id` = ?, `is_set` = ?, `creator` = ?, `date_created` = ?, `version` = ?, `changed_by` = ?, `date_changed` = ?, `retired_by` = ?, `date_retired` = ?, `retire_reason` = ?, `uuid` = ? WHERE concept_2.concept_id = ? ";
	}
	...
	public void copyFrom(EtlDatabaseObject toCopyFrom) {
		if (toCopyFrom instanceof ConceptVO) {
			ConceptVO toCopyFromAsConceptVO = (ConceptVO) toCopyFrom;

			this.conceptId = copyGeneratedField(toCopyFromAsConceptVO.conceptId);
			this.retired = copyGeneratedField(toCopyFromAsConceptVO.retired);
			this.shortName = copyGeneratedField(toCopyFromAsConceptVO.shortName);
			this.description = copyGeneratedField(toCopyFromAsConceptVO.description);
			this.formText = copyGeneratedField(toCopyFromAsConceptVO.formText);
			this.datatypeId = copyGeneratedField(toCopyFromAsConceptVO.datatypeId);
			this.classId = copyGeneratedField(toCopyFromAsConceptVO.classId);
			this.isSet = copyGeneratedField(toCopyFromAsConceptVO.isSet);
			this.creator = copyGeneratedField(toCopyFromAsConceptVO.creator);
			this.dateCreated = toCopyFromAsConceptVO.dateCreated;
			this.version = copyGeneratedField(toCopyFromAsConceptVO.version);
			this.changedBy = copyGeneratedField(toCopyFromAsConceptVO.changedBy);
			this.dateChanged = toCopyFromAsConceptVO.dateChanged;
			this.retiredBy = copyGeneratedField(toCopyFromAsConceptVO.retiredBy);
			this.dateRetired = copyGeneratedField(toCopyFromAsConceptVO.dateRetired);
			this.retireReason = copyGeneratedField(toCopyFromAsConceptVO.retireReason);
