...
public class PrivilegeVO extends AbstractDatabaseObject implements EtlDatabaseObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		super.load(rs);
			...
			String privilegeAttName = utilities.concatStringsWithSeparator(this.getRelatedConfiguration().getAlias(), "privilege", "_" );
	...
	public Object[]  getUpdateParams(){ 
 		Object[] params = {this.privilege.getValue(), this.description.getValue(), this.uuid, this.privilege.getValue()};
		return params; 
	} 
	...
	public String getUpdateSQL(){ 
 		return "UPDATE privilege SET `privilege` = ?, `description` = ?, `uuid` = ? WHERE privilege_6.privilege = ? "; 
	} 
 
	@JsonIgnore
	@Override
	public String generateInsertValuesWithoutObjectId(){ 
 		return ""+(this.description.getValue() != null ? "\""+ utilities.scapeQuotationMarks(this.description.getValue().toString())  +"\"" : null) + "," + (this.uuid != null ? "\""+ utilities.scapeQuotationMarks(this.uuid.toString())  +"\"" : null); 
	} 
 
	@JsonIgnore
	@Override
	public String generateInsertValuesWithObjectId(){ 
 		return ""+(this.privilege.getValue() != null ? "\""+ utilities.scapeQuotationMarks(this.privilege.getValue().toString())  +"\"" : null) + "," + (this.description.getValue() != null ? "\""+ utilities.scapeQuotationMarks(this.description.getValue().toString())  +"\"" : null) + "," + (this.uuid != null ? "\""+ utilities.scapeQuotationMarks(this.uuid.toString())  +"\"" : null); 
	} 
	...
	public void copyFrom(EtlDatabaseObject toCopyFrom){ 
 		if (toCopyFrom instanceof PrivilegeVO){
	    	PrivilegeVO toCopyFromAsPrivilegeVO = (PrivilegeVO)toCopyFrom;

			this.privilege = copyGeneratedField(toCopyFromAsPrivilegeVO.privilege);
			this.description = copyGeneratedField(toCopyFromAsPrivilegeVO.description);
