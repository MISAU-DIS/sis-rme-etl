...
public class UsersVO extends AbstractDatabaseObject implements EtlDatabaseObject {
	...
	public String getUpdateSQL() {
		return "UPDATE users SET `user_id` = ?, `system_id` = ?, `username` = ?, `password` = ?, `salt` = ?, `secret_question` = ?, `secret_answer` = ?, `creator` = ?, `date_created` = ?, `changed_by` = ?, `date_changed` = ?, `person_id` = ?, `retired` = ?, `retired_by` = ?, `date_retired` = ?, `retire_reason` = ?, `uuid` = ?, `creator_id` = ?, `activation_key` = ?, `email` = ? WHERE users_8.user_id = ? ";
	}
	...
	public void copyFrom(EtlDatabaseObject toCopyFrom) {
		if (toCopyFrom instanceof UsersVO) {
			...
			this.creatorId = copyGeneratedField(toCopyFromAsUsersVO.creatorId);
