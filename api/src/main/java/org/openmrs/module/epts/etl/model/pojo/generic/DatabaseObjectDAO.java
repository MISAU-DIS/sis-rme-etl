...
public class DatabaseObjectDAO extends BaseDAO {
	...
	public static EtlDatabaseObject thinGetByRecordOrigin(Integer recordOriginId, String recordOriginLocationCode,
		...
		try {
			...
			return find(parentTableConfiguration.getLoadHealper(),
					parentTableConfiguration.generateEtlRecordClass(parentTableConfiguration.getRelatedConnInfo()), sql,
					params, conn);
		} catch (Exception e) {
	...
	public static EtlDatabaseObject getDefaultRecord(TableConfiguration tableConfiguration, Connection conn)
		...
		return find(tableConfiguration.getLoadHealper(),
				tableConfiguration.generateEtlRecordClass(tableConfiguration.getRelatedConnInfo()), sql, params, conn);
	}
	...
	private static EtlDatabaseObject getExtremeNeverProcessedRecordOnOrigin(TableConfiguration tabConf, String function,
		...
		return find(tabConf.getLoadHealper(), tabConf.generateEtlRecordClass(tabConf.getRelatedConnInfo()), sql, params,
				conn);
	}
	...
	public static Integer countAllOfParentId(Class<? extends EtlDatabaseObject> clazz, String parentField,
			Integer parentId, Connection conn) throws DBException {
	...
	public static void insert(EtlDatabaseObject record, TableConfiguration tabConf, Connection conn)
			throws DBException {
	...
	public static Integer getSpecificRecord(EtlDatabaseObjectSearchParams searchParams, String function,
			Connection conn) throws DBException, ForbiddenOperationException {

		SearchClauses<EtlDatabaseObject> searchClauses = searchParams.generateSearchClauses(null, null, null, conn,
				null);

		searchClauses
				.setColumnsToSelect(function + "(" + searchParams.getConfig().getSrcConf().getPrimaryKey() + ") value");
	...
	private static GenericDatabaseObject getOutDatedRecordInDestination(TableConfiguration tableConfiguration,
		...
		String tablesToSelect = stageTable + " src_ INNER JOIN " + tableConfiguration.generateTableNameWithAlias()
				+ "  ON " + tableConfiguration.getTableAlias() + ".uuid = src_.record_uuid";
	...
	private static EtlDatabaseObject getPhantomRecordInDestination(TableConfiguration tableConfiguration,
		...
		String tablesToSelect = stageTable + " src_ RIGHT JOIN " + tableConfiguration.generateTableNameWithAlias()
				+ "  on " + tableConfiguration.getTableAlias() + ".uuid = src_.record_uuid";

		if (tableConfiguration.useSharedPKKey()) {
			tablesToSelect += "\n RIGHT JOIN "
					+ tableConfiguration.getSharedTableConf(conn).generateTableNameWithAlias() + " ON "
